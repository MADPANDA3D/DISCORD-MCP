"""Provider-owned deterministic ToolManifest for the packaged Discord MCP.
...
_IMAGE_REFERENCE_PATTERN = re.compile(
    r"^ghcr\.io/madpanda3d/discord-mcp-server@sha256:[0-9a-f]{64}$"
)
...
def _d(
    ...
    *,
    ...
    if open_world:
        operation = f"This {effect} through Discord's API and returns {result}."
        prerequisite = (
            "It requires a configured Discord bot and guild with the relevant permissions."
        )
    else:
        operation = f"This {effect} and returns {result}."
        prerequisite = (
            "It requires an authenticated MCP service request, but does not contact Discord."
        )
    return ToolDefinition(
        native_tool_name=name,
        aliases=aliases,
        title=title,
        description=(f"Use this when {use}.{avoid_sentence} {operation} {prerequisite}"),
        category=category,
        annotations={
            "readOnlyHint": read,
...
_DEFINITIONS = (
    _d(
        ...
        "server",
        "you need identity and high-level statistics for the configured server",
        "reads guild metadata without changing state",
        "guild identity, owner, creation date, channel/member counts, and boost status",
        aliases=("server_info", "guild_info"),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "configuration",
        "you need a live permission and readiness check for the configured guild",
        "reads bot, guild, channel, permission, and rate-limit state",
        "a red/yellow/green report, safe configuration flags, warnings, and capabilities",
        aliases=("health_check", "check_discord_health"),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    _d(
        "discord_ack",
        "Send Discord Acknowledgement",
        "messages",
        "you need to post a short standardized acknowledgement in an allowed channel",
        "creates one guild message",
        "the channel ID, created message ID, and jump URL",
        aliases=("acknowledge", "send_ack"),
        confirm=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
    ...
    _d(
        "discord_smoke_test",
        "Run Discord Write Smoke Test",
        "operations",
        "an operator needs an end-to-end write diagnostic",
        "may send, edit, read back, and delete a test message",
        "step-by-step health, write, read-back, and cleanup evidence",
        aliases=("smoke_test",),
        avoid="for routine health checks because it changes state",
        destructive=True,
        confirm=True,
        admin=True,
        tier="legacy",
    ...
    _d(
        ...
        "operations",
        "a legacy automation must run a supported action asynchronously",
        "queues behavior selected through a generic action field",
        "a task ID, queued status, and selected action",
        aliases=("submit_job",),
        avoid="when a typed direct tool is available",
        tier="legacy",
    ),
    _d(
        "discord_job_status",
        "Get Legacy Discord Job Status",
        "operations",
        "you need status or an optional result for discord_job_submit",
        "reads the in-memory legacy job record without changing Discord",
        "job status, timing, action, and an optional result or error",
        aliases=("job_status",),
        read=True,
        idempotent=True,
        open_world=False,
    ...
    _d(
        ...
        "messages",
        "you need to replace an existing message's content",
        "overwrites a message after admin and confirmation checks",
        "message/channel IDs, updated content, jump URL, or a dry-run plan",
        aliases=("update_message",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=MESSAGE_DOCS,
    ...
    _d(
        ...
        "messages",
        "you need to permanently remove an existing message",
        "deletes a message after admin and confirmation checks",
        "deleted message/channel IDs or a dry-run plan",
    ...
    _d(
        ...
        "messages",
        "you need recent messages from one readable channel",
        "reads channel history without changing state",
        "message IDs, authors, timestamps, content, embeds, attachments, reactions, and pagination context",
        aliases=("list_messages", "get_messages"),
        read=True,
        idempotent=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        ...
        "messages",
        "you need history filtered by text, author, date, link, file, or thread",
        "reads matching channel history without changing state",
        "matching messages, applied filters, counts, and pagination context",
        aliases=("find_messages", "message_search"),
        read=True,
        idempotent=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        ...
        "messages",
        "you need OCR or visual analysis of an attached image",
        "reads the attachment and sends it to the configured OpenAI vision endpoint",
    ...
    _d(
        "list_threads",
        "List Discord Threads",
        "threads",
        "you need active and optionally archived threads under a channel",
        "reads thread metadata without changing state",
        "thread IDs, names, archive state, parent channel, and counts",
        aliases=("get_threads",),
        read=True,
        idempotent=True,
        docs=CHANNEL_DOCS,
    ...
    _d(
        ...
        "threads",
        "you need to start a thread from an existing message",
        "creates a thread under the specified channel and message",
        "the new thread ID, name, parent channel, and archive duration",
        aliases=("start_thread",),
        confirm=True,
        docs=CHANNEL_DOCS,
    ),
    _d(
        ...
        "threads",
        "you need to close an active thread while preserving history",
        "changes a thread to archived state",
        "thread ID, name, and archived state",
        aliases=("close_thread",),
        destructive=True,
        confirm=True,
        docs=CHANNEL_DOCS,
    ),
    _d(
        ...
        "threads",
        "you need to reopen an archived thread",
        "changes a thread to active state",
        "thread ID, name, and archived state",
        aliases=("reopen_thread",),
        confirm=True,
        docs=CHANNEL_DOCS,
    ),
    _d(
        ...
        "audits",
        "you need a categorized summary of one channel day",
        "reads bounded channel history without changing state",
        "audit totals, classifications, samples, date bounds, and warnings",
        aliases=("daily_channel_audit",),
        read=True,
        idempotent=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        ...
        "audits",
        "you need a resumable audit across multiple channels",
        "creates an in-memory audit cursor without changing Discord",
        "task ID, channel totals, date, timezone, and status",
        aliases=("submit_daily_audit",),
        open_world=False,
    ),
    _d(
        ...
        "audits",
        "you need progress or accumulated results for a daily audit",
        "reads an in-memory audit record without contacting Discord",
        "cursor, status, completed counts, timing, and optional results",
        aliases=("daily_audit_status",),
        read=True,
        idempotent=True,
        open_world=False,
    ),
    _d(
        ...
        "audits",
        "you need to advance a resumable audit by one channel",
        "reads one channel and advances the in-memory cursor",
        "the processed audit, updated cursor, progress, and task status",
        aliases=("next_daily_audit",),
        docs=MESSAGE_DOCS,
    ),
    _d(
        "add_reaction",
        ...
        "reactions",
        "you need the bot to react to an existing message",
        "adds one emoji reaction",
        "channel/message IDs and the applied emoji",
        aliases=("react_to_message",),
        confirm=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        ...
        "reactions",
        "you need to remove the bot's reaction from a message",
        "removes one emoji reaction",
        "channel/message IDs and the removed emoji",
        aliases=("unreact_to_message",),
        destructive=True,
        confirm=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        "timeout_member",
        "Timeout Discord Member",
        "moderation",
        "an authorized moderator must temporarily restrict a member",
        "sets a communication timeout after hierarchy checks",
        "member ID, expiry, duration, and reason",
        aliases=("mute_member",),
        destructive=True,
        confirm=True,
        admin=True,
    ...
    _d(
        "remove_timeout",
        "Remove Discord Member Timeout",
        "moderation",
        "an authorized moderator must restore a timed-out member",
        "clears the member timeout after hierarchy checks",
        "member ID and moderation reason",
        aliases=("untimeout_member", "unmute_member"),
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "Kick Discord Member",
        "moderation",
        "an authorized moderator must remove a member without banning re-entry",
        "removes the member after hierarchy checks",
        "kicked member ID and reason",
        aliases=("remove_member",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ...
    _d(
        ...
        "Ban Discord Member",
        "moderation",
        "an authorized moderator must prohibit a user from joining",
        "bans the member and can delete bounded recent messages",
        "banned member ID, deleted-message days, and reason",
        aliases=("block_member",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ...
    _d(
        ...
        "Unban Discord User",
        "moderation",
        "an authorized moderator must allow a banned user to rejoin",
        "removes a guild ban",
        "unbanned user ID and reason",
        aliases=("remove_ban",),
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ...
    _d(
        ...
        "roles",
        "an authorized operator must grant a configured role",
        "adds one role after hierarchy checks",
        "member ID, role ID, and reason",
        aliases=("grant_role",),
        destructive=True,
        confirm=True,
        admin=True,
    ...
    _d(
        ...
        "roles",
        "an authorized operator must revoke a configured role",
        "removes one role after hierarchy checks",
        "member ID, role ID, and reason",
        aliases=("revoke_role",),
        destructive=True,
        confirm=True,
    ...
    _d(
        "edit_nickname",
        "Edit Discord Member Nickname",
        "members",
        "an authorized operator must set or clear a nickname",
        "overwrites the guild nickname after hierarchy checks",
        "member ID, resulting nickname, and reason",
        aliases=("set_nickname",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ...
    _d(
        "get_user_id_by_name",
        "Find Discord User ID by Name",
        "members",
        "you need a member snowflake from a username or display name",
        "searches guild members without changing state",
        "matching IDs, usernames, display names, and match quality",
        aliases=("find_user", "resolve_user_id"),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    ...
    _d(
        ...
        "direct_messages",
        "you need to send a private message and DM access is enabled",
        "creates one direct message",
        "recipient/channel/message IDs and jump URL",
        aliases=("send_dm",),
        avoid="for guild channels; use send_message",
        confirm=True,
        docs=MESSAGE_DOCS,
    ),
    _d(
        "edit_private_message",
        "Edit Discord Direct Message",
        "direct_messages",
        "you need to replace a bot-authored direct message",
        "overwrites an existing direct message",
        "recipient/channel/message IDs, content, and jump URL",
        aliases=("edit_dm",),
        destructive=True,
        confirm=True,
        docs=MESSAGE_DOCS,
    ...
    _d(
        ...
        "Delete Discord Direct Message",
        "direct_messages",
        "you need to permanently remove a bot-authored direct message",
        "deletes an existing direct message",
        "recipient/channel/deleted-message IDs",
        aliases=("delete_dm",),
        destructive=True,
        confirm=True,
        docs=MESSAGE_DOCS,
    ...
    _d(
        ...
        "direct_messages",
        "you need recent DM history and DM access is enabled",
        "reads direct-message history without changing state",
        "recipient/channel IDs and recent message records",
        aliases=("read_dms",),
        read=True,
        idempotent=True,
        docs=MESSAGE_DOCS,
    ...
    _d(
        ...
        "channels",
        "you need a new text channel in the guild or category",
        "creates a guild text channel",
        "new channel ID, name, guild ID, and category ID",
        aliases=("add_channel",),
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ...
    _d(
        ...
        "channels",
        "you need to permanently remove a channel",
        "deletes the guild channel",
        "deleted channel ID, name, and guild ID",
    ...
    _d(
        ...
        "channels",
        "you need a channel ID from a name",
        "searches readable guild channels without changing state",
        "matching IDs, names, types, and category context",
        aliases=("resolve_channel",),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "channels",
        "you need the readable channel inventory",
        "reads guild channels without changing state",
        "channel IDs, names, types, positions, and category context",
        aliases=("get_channels",),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "categories",
        "you need a new category to organize channels",
        "creates a guild channel category",
        "new category ID, name, and guild ID",
        aliases=("add_category",),
        confirm=True,
        admin=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "categories",
        "you need to remove a category after handling child channels",
        "deletes the category object",
        "deleted category ID, name, and guild ID",
        aliases=("remove_category",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=CHANNEL_DOCS,
    ...
    _d(
        ...
        "categories",
        "you need a category ID from its name",
        "searches guild categories without changing state",
        "matching category IDs, names, positions, and guild ID",
        aliases=("resolve_category",),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    _d(
        ...
        "List Discord Category Channels",
        "categories",
        "you need readable children of one category",
        "reads category membership without changing state",
        "category ID plus channel IDs, names, types, and count",
        aliases=("category_channels",),
        read=True,
        idempotent=True,
        docs=GUILD_DOCS,
    ),
    ...
    _d(
        ...
        "webhooks",
        "you need to permanently remove a known webhook by ID",
        "deletes the webhook",
        "deleted webhook ID and name",
        aliases=("remove_webhook",),
        destructive=True,
        confirm=True,
        admin=True,
        docs=WEBHOOK_DOCS,
    ...
    _d(
        ...
        "webhooks",
        "an authorized operator needs credential-safe webhook inventory for one readable channel",
        "reads webhook metadata without returning token-bearing URLs",
        "webhook IDs and names only",
    ...
    _d(
        "check_configuration",
        "Check Discord MCP Configuration",
        "navigation",
        "you need to know whether this request has required provider setup",
        "checks safe configuration presence and policy flags",
        "readiness, missing field names, capability flags, and warnings without credential values",
        aliases=("configuration_status",),
        read=True,
        idempotent=True,
        open_world=False,
    ...
    _d(
        ...
        "navigation",
        "you need capability groups, counts, or the lossless ToolManifest",
        "reads the provider-owned catalog from memory",
        "catalog identity, counts, categories, and optionally every descriptor",
        aliases=("get_manifest", "list_tools_manifest"),
        read=True,
        idempotent=True,
        open_world=False,
        navigation="catalog",
    ...
    _d(
        ...
        "Get Discord Endpoint Coverage",
        "navigation",
        "you need to understand covered and excluded Discord API areas",
        "reads the maintained coverage matrix from memory",
        "official links, coverage state, tools, configuration, and gap reasons",
        aliases=("endpoint_coverage",),
        read=True,
        idempotent=True,
        open_world=False,
        navigation="coverage",
    ...
    _d(
        "get_tool_usage",
        "Get Discord Tool Usage",
        "navigation",
        "you need the lossless descriptor for one tool",
        "reads one canonical descriptor from memory",
        "complete parameters, output, risk, confirmation, and follow-up guidance",
        aliases=("describe_tool", "tool_reference"),
        read=True,
        idempotent=True,
        open_world=False,
    ...
    _d(
        ...
        "Find Discord Tools",
        "navigation",
        "you need ranked tools for a multi-word task or alias",
        "searches the deterministic catalog in memory",
        "compact ranked matches with risk, category, tier, and next action",
...
PARAMETER_DESCRIPTIONS = {
    ...
    "attachment_index": "Zero-based decimal index of the attachment on the message.",
    "byte_offset": "Zero-based attachment byte offset returned by next_byte_offset; zero starts a new bounded download.",
...
ENDPOINT_COVERAGE = (
    ...
    },
    {
        ...
        "documentationUrl": REPOSITORY_DOCS_URL,
        "status": "provider_extension",
        "configuration": ["Discord bot token", "guild ID", "channel read policy"],
        "tools": [
            "channel_daily_audit",
            "daily_audit_job_submit",
            "daily_audit_job_status",
...
def get_source_fingerprint(value: str | None = None) -> str:
    """Return a source SHA-256 or a non-sensitive sentinel."""

    candidate = value if value is not None else os.getenv("MCP_SOURCE_FINGERPRINT", "")
    candidate = str(candidate or "").strip()
    if candidate == "development" or _SOURCE_FINGERPRINT_PATTERN.fullmatch(candidate):
        return candidate
    return "unknown"


def get_image_reference(value: str | None = None) -> str:
    """Return the immutable fleet image reference or a non-sensitive sentinel."""

    candidate = value if value is not None else os.getenv("MCP_IMAGE_REFERENCE", "")
    candidate = str(candidate or "").strip()
    if candidate == "development" or _IMAGE_REFERENCE_PATTERN.fullmatch(candidate):
        return candidate
    return "unknown"
    ...
    return {
        ...
        "meta": {
            "madpanda": {
                ...
                "adminRequired": definition.access["adminRequired"],
            }
        },
    ...
    success = {
        ...
        "properties": {
            ...
            "data": {
                ...
                "properties": {
                    field_name: _output_field_schema(tool_name, field_name) for field_name in fields
                },
                "additionalProperties": True,
            },
        ...
        canonical_descriptors.append(
            {
                ...
                "outputSchema": _provider_output_schema(name, definition.output_data_description),
                "annotations": dict(definition.annotations),
                "access": dict(definition.access),

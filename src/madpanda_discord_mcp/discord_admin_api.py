...
_OPERATIONS = (
    ...
    _operation(
        "get_role_member_counts", "GET", "/guilds/{guild_id}/roles/member-counts", "read", "role"
    ),
    _operation(
        "get_prune_count",
        "GET",
    ...
    _operation(
        ...
        "read",
        "message",
        query=("around", "before", "after", "limit"),
    ),
    _operation(
    ...
    _operation(
        "get_message", "GET", "/channels/{channel_id}/messages/{message_id}", "read", "message"
    ),
    _operation(
        "get_reactions",
        "GET",
    ...
    _operation(
        "get_current_voice_state", "GET", "/guilds/{guild_id}/voice-states/@me", "read", "voice"
    ),
    _operation(
    ...
    _operation(
        "get_guild_sticker", "GET", "/guilds/{guild_id}/stickers/{sticker_id}", "read", "sticker"
    ),
    _operation(
        "list_default_soundboard_sounds", "GET", "/soundboard-default-sounds", "read", "soundboard"
    ),
    _operation(
    ...
    _operation(
        "get_guild_template", "GET", "/guilds/templates/{template_code}", "read", "template"
    ),
    ...
    _operation(
        ...
        required_body=("webhook_channel_id",),
        reason=True,
    ),
    ...
    _operation(
        ...
        "write",
        "thread",
        body=("name", "auto_archive_duration", "rate_limit_per_user"),
        required_body=("name",),
        reason=True,
    ),
    _operation(
    ...
    _operation(
        ...
        "write",
        "thread",
        body=("name", "auto_archive_duration", "rate_limit_per_user", "message", "applied_tags"),
        required_body=("name", "message"),
        reason=True,
    ),
    _operation(
        "join_thread", "PUT", "/channels/{channel_id}/thread-members/@me", "write", "thread"
    ),
    _operation(
        "add_thread_member",
        "PUT",
    ...
    _operation(
        ...
        required_body=("name", "event_type", "trigger_type", "actions"),
        reason=True,
    ),
    _operation(
        ...
        required_body=("name", "privacy_level", "scheduled_start_time", "entity_type"),
        reason=True,
    ),
    _operation(
        ...
        required_body=("channel_id", "topic"),
        reason=True,
    ),
    _operation(
        "create_guild_emoji",
        ...
        body=("name", "image", "roles"),
        required_body=("name", "image"),
        reason=True,
    ),
    _operation(
        ...
        "sticker",
        permission="manage_guild_expressions",
        body=("name", "description", "tags", "file_base64", "filename"),
        required_body=("name", "tags", "file_base64"),
        reason=True,
        body_mode="sticker_multipart",
    ),
    _operation(
    ...
    _operation(
        ...
        "soundboard",
        permission="manage_guild_expressions",
        body=("name", "sound", "volume", "emoji_id", "emoji_name"),
        required_body=("name", "sound"),
        reason=True,
    ),
    _operation(
        "create_webhook_safe",
    ...
    _operation(
        ...
        "moderation",
        permission="kick_members",
        body=("days", "compute_prune_count", "include_roles"),
        reason=True,
    ),
    _operation(
        "delete_integration",
    ...
    _operation(
        ...
        "guild",
        permission="manage_guild",
        body=("prompts", "default_channel_ids", "enabled", "mode"),
        reason=True,
    ),
    _operation(
        ...
        "guild",
        permission="manage_guild",
        body=("invites_disabled_until", "dms_disabled_until"),
        reason=True,
    ),
    _operation(
        "modify_channel",
    ...
    _operation(
        ...
        "destructive",
        "voice",
        body=("channel_id", "suppress", "request_to_speak_timestamp"),
    ),
    _operation(
    ...
    _operation(
        ...
        "soundboard",
        permission="manage_guild_expressions",
        body=("name", "volume", "emoji_id", "emoji_name"),
        reason=True,
    ),
    _operation(
...
def _identifier(value: Any, name: str) -> str:
    ...
    if name not in {"emoji", "invite_code", "template_code"} and not _SNOWFLAKE.fullmatch(
        normalized
    ):
        raise ValueError(f"{name} must be a Discord snowflake.")
    if name in {"invite_code", "template_code"} and not re.fullmatch(
        r"[A-Za-z0-9_-]{2,100}", normalized
    ):
...
def validate_query(
    operation: DiscordAdminOperation, query: Mapping[str, Any] | None
) -> dict[str, Any]:
...
def validate_payload(
    operation: DiscordAdminOperation, payload: Mapping[str, Any] | None
) -> dict[str, Any]:
    ...
    if operation.action == "bulk_delete_messages":
        ...
        if not isinstance(messages, list) or not 2 <= len(messages) <= 100:
            raise ValueError(
                "bulk_delete_messages payload.messages must contain 2 to 100 snowflakes."
            )
...
def bound_response(value: Any) -> Any:
    safe = _redact(value)
    if (
        len(json.dumps(safe, separators=(",", ":"), ensure_ascii=False).encode("utf-8"))
        <= MAX_RESPONSE_BYTES
    ):
        return safe
                ...
                if response.status < 200 or response.status >= 300:
                    ...
                    return {
                        ...
                        "error": {
                            ...
                            "message": str(
                                message or "Discord rejected the server-management request."
                            )[:512],

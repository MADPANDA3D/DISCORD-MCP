...
SERVER_MANAGEMENT_DOCS = "https://docs.discord.com/developers/platform/server-and-channel-management"
...
_DEFINITIONS = (
    ...
    _d("discord_server_read", "Read Discord Server Management API", "server_management", "you need one officially documented bot-token server administration read for guilds, roles, members, effective channel permissions, invites, forums, threads, messages, pins, AutoMod, audit logs, scheduled events, voice, stages, expressions, soundboards, webhooks, or templates", "executes one immutable allowlisted GET action with typed identifiers and bounded pagination", "the selected action, resource, HTTP status, redacted bounded provider result, rate-limit state, and audit trace", aliases=("read_server_management", "list_members", "list_roles", "effective_permissions", "audit_logs", "list_forum_threads", "list_invites", "list_automod", "list_scheduled_events"), read=True, idempotent=True, docs=SERVER_MANAGEMENT_DOCS),
    _d("discord_server_write", "Write Discord Server Management API", "server_management", "you need one additive or reversible official bot-token administration action such as creating a channel, role, invite, forum post, AutoMod rule, scheduled event, stage, expression, sound, safe webhook, or template", "executes one immutable allowlisted write action after provider policy and permission checks", "the selected action, resource, HTTP status, credential-redacted provider result, rate-limit state, and audit trace", aliases=("create_role", "create_forum_post", "create_invite", "create_automod", "create_scheduled_event", "create_stage", "create_webhook_safe"), confirm=True, docs=SERVER_MANAGEMENT_DOCS),
    _d("discord_server_destructive", "Change or Delete Discord Server State", "server_management", "you need one official bot-token overwrite, reorder, moderation, permission, bulk-delete, prune, removal, or delete action", "executes one immutable allowlisted destructive action only after exact confirmation, provider policy, role hierarchy, permission, channel, and protected-target checks", "the selected action, resource, HTTP status, credential-redacted provider result, rate-limit state, and audit trace", aliases=("modify_role", "delete_role", "edit_permissions", "bulk_moderation", "reorder_channels", "manage_voice", "delete_automod", "delete_scheduled_event"), destructive=True, confirm=True, docs=SERVER_MANAGEMENT_DOCS),
...
_OBJECT_OUTPUT_FIELDS = {
    ...
    "nextAction", "range_utc", "result", "results", "usage",
}

ENDPOINT_COVERAGE = (
    {"feature": "guild-settings-and-community", "documentationUrl": GUILD_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "MANAGE_GUILD where required"], "tools": ["get_server_info", "discord_health_check", "discord_server_read", "discord_server_destructive"], "actions": ["get_guild", "get_guild_preview", "get_widget_settings", "get_widget", "get_vanity_url", "get_welcome_screen", "get_onboarding", "modify_guild", "modify_widget", "modify_welcome_screen", "modify_onboarding", "modify_incident_actions"], "notes": "Stable bot-token guild metadata, settings, widgets, onboarding, welcome-screen, and incident actions are allowlisted. Owner-only MFA transfer and owner transfer remain provider-enforced."},
    {"feature": "channels-categories-permissions-and-ordering", "documentationUrl": CHANNEL_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "channel policy", "MANAGE_CHANNELS or MANAGE_ROLES where required"], "tools": ["list_channels", "create_text_channel", "delete_channel", "discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_guild_channels", "get_channel", "get_effective_channel_permissions", "create_guild_channel", "modify_channel", "delete_channel", "modify_channel_positions", "edit_channel_permissions", "delete_channel_permission", "set_voice_channel_status"], "notes": "Covers text, category, announcement, voice, stage, forum, and media channel objects, positions, raw overwrites, effective bot/member/role permissions, and permission preflight."},
    {"feature": "messages-reactions-pins-and-bulk-moderation", "documentationUrl": MESSAGE_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "MESSAGE_CONTENT intent when content is required", "channel policy"], "tools": ["send_message", "edit_message", "delete_message", "read_messages", "search_messages", "add_reaction", "remove_reaction", "discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["get_channel_messages", "search_guild_messages", "get_message", "get_reactions", "get_channel_pins", "crosspost_message", "pin_message", "unpin_message", "delete_all_reactions", "delete_reactions_for_emoji", "bulk_delete_messages", "trigger_typing"], "notes": "Stable guild message administration includes current paginated pin routes. Poll voting and interaction callbacks are user/application interaction features, not server administration."},
    {"feature": "forums-threads-and-membership", "documentationUrl": CHANNEL_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "thread and channel permissions"], "tools": ["list_threads", "create_thread", "archive_thread", "unarchive_thread", "discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_active_threads", "start_thread_from_message", "start_thread_without_message", "start_forum_thread", "join_thread", "add_thread_member", "leave_thread", "remove_thread_member", "get_thread_member", "list_thread_members", "list_public_archived_threads", "list_private_archived_threads", "list_joined_private_archived_threads"], "notes": "Forum/media post creation and complete stable thread membership and archive inventory are covered."},
    {"feature": "members-roles-bans-prune-and-voice-state", "documentationUrl": GUILD_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "GUILD_MEMBERS intent where required", "moderation permissions", "protected target policy"], "tools": ["get_user_id_by_name", "timeout_member", "remove_timeout", "kick_member", "ban_member", "unban_member", "add_role", "remove_role", "edit_nickname", "discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["get_member", "list_members", "search_members", "list_bans", "get_ban", "list_roles", "get_role", "get_role_member_counts", "get_prune_count", "list_guild_voice_regions", "get_current_voice_state", "get_user_voice_state", "add_member_role", "create_role", "modify_member", "modify_current_member", "modify_current_nick", "remove_member_role", "bulk_ban", "modify_role_positions", "modify_role", "delete_role", "begin_prune", "modify_current_voice_state", "modify_user_voice_state"], "notes": "Role lifecycle and ordering, bulk member inventory, protected-target checks, role hierarchy, bulk bans, prune, and guild voice-state management are covered. Add Guild Member is excluded because it requires a user OAuth2 access token with guilds.join, not a bot token alone."},
    {"feature": "invites-integrations-and-templates", "documentationUrl": GUILD_DOCS, "status": "covered", "configuration": ["Discord bot token", "guild ID", "invite, channel, integration, and guild permissions"], "tools": ["discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_guild_invites", "list_channel_invites", "get_vanity_url", "create_channel_invite", "delete_invite", "list_integrations", "delete_integration", "get_guild_templates", "get_guild_template", "create_guild_template", "sync_guild_template", "modify_guild_template", "delete_guild_template"], "notes": "Stable bot-token guild invite, integration, and template management is covered. Community invite target-user jobs are campaign analytics rather than server structure and are excluded."},
    {"feature": "automod-and-audit-logs", "documentationUrl": "https://docs.discord.com/developers/resources/auto-moderation", "status": "covered", "configuration": ["Discord bot token", "guild ID", "MANAGE_GUILD and VIEW_AUDIT_LOG"], "tools": ["discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_automod_rules", "get_automod_rule", "create_automod_rule", "modify_automod_rule", "delete_automod_rule", "get_audit_log"], "notes": "Auto Moderation CRUD and bounded 45-day guild audit-log reads are covered with audit reasons on supported mutations."},
    {"feature": "scheduled-events-stage-and-voice", "documentationUrl": "https://docs.discord.com/developers/resources/guild-scheduled-event", "status": "covered", "configuration": ["Discord bot token", "guild ID", "MANAGE_EVENTS and stage/voice permissions"], "tools": ["discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_scheduled_events", "get_scheduled_event", "list_scheduled_event_users", "create_scheduled_event", "modify_scheduled_event", "delete_scheduled_event", "get_stage_instance", "create_stage_instance", "modify_stage_instance", "delete_stage_instance", "list_voice_regions", "get_current_voice_state", "get_user_voice_state", "modify_current_voice_state", "modify_user_voice_state"], "notes": "Stable scheduled-event, stage-instance, voice-region, and guild voice-state endpoints are covered."},
    {"feature": "emojis-stickers-and-soundboard", "documentationUrl": "https://docs.discord.com/developers/resources/emoji", "status": "covered", "configuration": ["Discord bot token", "guild ID", "guild expression and soundboard permissions"], "tools": ["discord_server_read", "discord_server_write", "discord_server_destructive"], "actions": ["list_guild_emojis", "get_guild_emoji", "create_guild_emoji", "modify_guild_emoji", "delete_guild_emoji", "list_guild_stickers", "get_guild_sticker", "create_guild_sticker", "modify_guild_sticker", "delete_guild_sticker", "list_default_soundboard_sounds", "list_guild_soundboard_sounds", "get_guild_soundboard_sound", "send_soundboard_sound", "create_guild_soundboard_sound", "modify_guild_soundboard_sound", "delete_guild_soundboard_sound"], "notes": "Guild expression CRUD, bounded sticker multipart upload, and soundboard inventory/lifecycle are covered. Application-owned emojis are application configuration rather than guild administration."},
    {"feature": "credential-safe-webhook-management", "documentationUrl": WEBHOOK_DOCS, "status": "covered_with_documented_exclusion", "configuration": ["Discord bot token", "guild ID", "MANAGE_WEBHOOKS"], "tools": ["discord_server_read", "discord_server_write", "discord_server_destructive", "send_message"], "actions": ["list_channel_webhooks", "list_guild_webhooks", "get_webhook", "create_webhook_safe", "modify_webhook_safe", "delete_webhook_safe"], "notes": "Bot-authorized webhook management is covered and every credential-bearing token or URL is redacted. Webhook-token execution and token-authenticated message routes are excluded because exposing or accepting bearer-like webhook secrets would violate the MCP credential boundary; send_message is the safe delivery path."},
    {"feature": "direct-messages", "documentationUrl": CHANNEL_DOCS, "status": "covered_with_documented_exclusion", "configuration": ["Discord bot token", "DM enabled policy"], "tools": ["send_private_message", "edit_private_message", "delete_private_message", "read_private_messages"], "notes": "One-to-one bot DMs are covered when explicitly enabled. Group-DM recipient endpoints require user-account OAuth access and are not bot-token server administration."},
...
def _parameter_description(tool_name: str, parameter_name: str) -> str:
    if tool_name.startswith("discord_server_"):
        server_descriptions = {
            ...
            "query": "Action-specific bounded query parameters; unknown keys and limits above 100 are rejected.",

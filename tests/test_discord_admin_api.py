...
from madpanda_discord_mcp.discord_admin_api import (
    DESTRUCTIVE_ACTIONS,
    MAX_RESPONSE_BYTES,
    OPERATIONS,
    READ_ACTIONS,
...
class DiscordAdminApiTests(unittest.IsolatedAsyncioTestCase):
    def test_registry_is_unique_risk_partitioned_and_explicit(self):
        ...
        self.assertEqual(
            set(OPERATIONS), set(READ_ACTIONS) | set(WRITE_ACTIONS) | set(DESTRUCTIVE_ACTIONS)
        )
    ...
    def test_query_and_payload_are_bounded_and_allowlisted(self):
        ...
        with self.assertRaisesRegex(ValueError, "bot, member, or role"):
            validate_query(
                OPERATIONS["get_effective_channel_permissions"], {"target_type": "everyone"}
            )
